...
conv_net = ConvNet()
conv_net.load_state_dict(torch.load('conv_net_model.ckpt'))
...
test_preds = MakePredictions(conv_net, xTest, batch_size=128) ## Make Predictions on test dataset

test_preds = torch.cat(test_preds) ## Combine predictions of all batches

test_preds = test_preds.argmax(dim=1)

train_preds = MakePredictions(conv_net, xTrain, batch_size=128) ## Make Predictions on train dataset
